...
const funcRegex = /const handlePostponeInstallment = \(e: React\.FormEvent\) => \{/;
...
`;
code = code.replace(funcRegex, handleInterestFunc + '\n  ' + "const handlePostponeInstallment = (e: React.FormEvent) => {");

// 3. Add Yapılandır button next to Ertele
const erteleButtonRegex = /<button\s+onClick=\{\(\) => \{\s+setPostponeForm\(\{\s+customerId: inst\.customerId,\s+installmentId: inst\.id,\s+oldDate: inst\.dueDate,\s+newDate: inst\.dueDate,\s+notifyCustomer: true,\s+description: inst\.description\s+\}\);\s+setIsPostponeModalOpen\(true\);\s+\}\}\s+className="px-3 py-1\.5 rounded-lg text-sm font-medium transition-colors bg-orange-50 text-orange-600 hover:bg-orange-100"\s+>\s+Ertele\s+<\/button>/;
...
// 4. Add Modal at the end of component
const modalTargetRegex = /\{\/\* Taksit Ertele Modal \*\/\}/;
...
console.log("Patched Taksitler.tsx with Interest/Restructure module");
