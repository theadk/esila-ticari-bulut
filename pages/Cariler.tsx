...
import { Plus, ChevronDown, MoreHorizontal, Search, Edit2, Trash2, Mail, Phone, MapPin, X, Save, Building, User, FileText, History, Download, CreditCard, Send, Upload, Printer, MessageCircle, MessageSquare, CheckCircle, Landmark, Mic, MicOff, Calendar } from 'lucide-react';
...
import { CRM } from './CRM';
import { CariRaporModal } from '../components/CariRaporModal';
...
export const Cariler: React.FC = () => {
  ...
  const [activeView, setActiveView] = useState<'liste' | 'hatirlatmalar'>('liste');
  ...
  const sendBulkMutabakatSMS = async () => {
    const selectedCustomers = customers.filter(c => selectedCustomerIds.includes(c.id) && c.phone);
    if (selectedCustomers.length === 0) {
      toast.error("Seçili carilerin geçerli telefon numarası bulunmuyor.");
      return;
    }
    if (!window.confirm(`${selectedCustomers.length} cariye mutabakat formu (SMS) gönderilecek. Onaylıyor musunuz?`)) return;
    
    toast.loading("Mutabakat SMS'leri gönderiliyor...", { id: 'bulkMutabakat' });
    let successCount = 0;
    let failCount = 0;
    
    for (const c of selectedCustomers) {
      const text = `Sayın ${c.companyName || c.name} yetkilisi, ${new Date().toLocaleDateString('tr-TR')} tarihi itibariyle bakiyemiz ${(c.balance || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL tutarında mutabıktır. ${store.settings?.companyName || 'Şirket'}`;
      try {
        await sendSMS(store.settings, [c.phone], text);
        successCount++;
      } catch(e) {
        failCount++;
      }
    }
    toast.success(`Gönderim tamamlandı. Başarılı: ${successCount}, Başarısız: ${failCount}`, { id: 'bulkMutabakat' });
  };

  const sendBulkPaymentReminderSMS = async () => {
    const selectedCustomers = customers.filter(c => selectedCustomerIds.includes(c.id) && c.phone);
    if (selectedCustomers.length === 0) {
      toast.error("Seçili carilerin geçerli telefon numarası bulunmuyor.");
      return;
    }
    if (!window.confirm(`${selectedCustomers.length} cariye ödeme hatırlatması (SMS) gönderilecek. Onaylıyor musunuz?`)) return;
    
    toast.loading("Ödeme Hatırlatma SMS'leri gönderiliyor...", { id: 'bulkReminder' });
    let successCount = 0;
    let failCount = 0;
    
    for (const c of selectedCustomers) {
      const text = `Sayın ${c.companyName || c.name}, güncel bakiye durumunuz: ${(c.balance || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL (${c.balance > 0 ? 'Borçlu' : 'Alacaklı'}). Detaylı bilgi için bizimle iletişime geçebilirsiniz. İyi çalışmalar dileriz. - ${store.settings?.companyName || 'Şirket'}`;
      try {
        await sendSMS(store.settings, [c.phone], text);
        successCount++;
      } catch(e) {
    ...
    toast.success(`Gönderim tamamlandı. Başarılı: ${successCount}, Başarısız: ${failCount}`, { id: 'bulkReminder' });
  };
  ...
  return (
    <div className="relative">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selectedCustomerIds.length > 0 && (
            <div className="relative group z-20">
              ...
              <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-100 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all flex flex-col overflow-hidden">
                <button 
                  onClick={() => setIsBulkWhatsAppOpen(true)}
                  className="px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center gap-2 border-b text-gray-700"
                >
                ...
                <button 
                  onClick={sendBulkMutabakatSMS}
                  className="px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center gap-2 border-b text-gray-700"
                >
                  <CheckCircle size={16} className="text-emerald-600" />
                  Toplu Mutabakat Formu Gönder (SMS)
                </button>
                <button 
                  onClick={sendBulkPaymentReminderSMS}
                  className="px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center gap-2 text-gray-700"
                >
                  <CreditCard size={16} className="text-orange-600" />
                  Toplu Ödeme Hatırlatması Gönder (SMS)
                </button>
              </div>
            </div>
          )}
      ...
      {activeTab === 'CRM' ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto print:border-none print:shadow-none print:overflow-visible">
          <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between gap-4 print:hidden">
                  ...
                  <tr key={customer.id} className={`hover:bg-emerald-50/30 transition-colors ${selectedCustomerIds.includes(customer.id) ? 'bg-emerald-50/50' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button 
                          title="Kapsamlı Cari Raporu"
                          onClick={() => { setSelectedCustomerForReport(customer); setIsReportModalOpen(true); }}
      ...
      {activeView === 'hatirlatmalar' && (
        <div className="space-y-6 animate-fade-in flex flex-col h-full bg-white p-6 rounded-xl border border-gray-100 shadow-sm mt-6">
                ...
                <tr>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-500 w-10 text-center">
                    <input 
                      ...
                      onChange={(e) => {
                        if (e.target.checked) setSelectedReminderIds(reminderCustomers.map((c: any) => c.id));
            ...
            <div className="p-4 sm:p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="space-y-3">
                {selectedReminderIds.map((id, index) => {
                  const customer = reminderCustomers.find((c: any) => c.id === id);
              ...
              <button 
                onClick={async () => {
                  const phones = selectedReminderIds
                    .map(id => reminderCustomers.find((c: any) => c.id === id)?.phone)
